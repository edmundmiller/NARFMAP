...
TEST(Cigar, Operations)
{
  using dragenos::align::Cigar;
  Cigar cigar;
  ASSERT_EQ(0u, cigar.getNumberOfOperations());
  ...
  ASSERT_EQ(1u, cigar.getNumberOfOperations());
  cigar.push_back(opI21);
  ASSERT_EQ(2u, cigar.getNumberOfOperations());
  cigar.push_back(Cigar::Operation(opD35.first, opD35.second));
  ASSERT_EQ(3u, cigar.getNumberOfOperations());
  ...
  ASSERT_EQ(17u, cigar.getOperations()->second);
  ASSERT_EQ(21u, (cigar.getOperations() + 1)->second);
  ASSERT_EQ(35u, (cigar.getOperations() + 2)->second);
  cigar.clear();
  ASSERT_EQ(0u, cigar.getNumberOfOperations());
}
