...
TEST(CrcHasher, KnownHashValues)
{
  ...
  ASSERT_EQ(primaryHasher.getHash64(0x3543543543), 0x35de20855c4d8eu);
  ASSERT_EQ(primaryHasher.getHash64(0x10d50d50d50), 0x2643a64efec778u);
  ASSERT_EQ(primaryHasher.getHash64(0x14354354354), 0x24e8498f420b6fu);
  ASSERT_EQ(primaryHasher.getHash64(0x150d50d50d5), 0x9287b35d36195u);
  ASSERT_EQ(primaryHasher.getHash64(0xd50d50d50d), 0x22d3a73e8851c7u);
}
