...
public:
  ...
  void showInterval(const size_t start, const size_t length,
                    const unsigned seedLength = 21,
                    const unsigned halfWing = 0) const;
...
void ExtendTableFixture::showInterval(const size_t start, const size_t length,
                                      const unsigned seedLength,
                                      const unsigned halfWing) const {
  const uint64_t *const extendTable = globalTestEnvironment->extendTable;
  const unsigned char *referenceData =
      globalTestEnvironment->getReferenceData();
  ...
  for (size_t i = 0; 12 > i; ++i) {
    ...
    std::cerr << std::hex << std::setfill('0') << std::setw(8)
              << (extendRecord >> 32) << ":" << std::setw(8) << position << ":";
    for (unsigned j = 0; j < seedLength; ++j)
      std::cerr << " " << (unsigned)getBase(referenceData, position + j);
    std::cerr << std::setfill(' ') << std::dec << std::endl;
  }
  std::cerr << std::endl;
  for (size_t i = 0; 12 > i; ++i) {
